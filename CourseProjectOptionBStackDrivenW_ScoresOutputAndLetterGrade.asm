...
LETTERGRADE	ST R7, SAVEREG7										; The LETTERGRADE subroutine that determines the letter grade of a score, R7 is backed up just in case since there's branching
		...
		LD R1, AGRADESCALE									
		ADD R3, R2, R1										; subtract 100 from a score
		BRzp LETTERA										; if zero or negative, the score was an A
		JSR PREPLETTER
		LD R1, BGRADESCALE
		ADD R3, R2, R1										; subtract 90 from a score
		BRzp LETTERB										; if zero or negative, the score was a B
		JSR PREPLETTER
		LD R1, CGRADESCALE
		ADD R3, R2, R1										; subtract 80 from a score
		BRzp LETTERC										; if zero or negative, the score was a C
		JSR PREPLETTER
		LD R1, DGRADESCALE
		ADD R3, R2, R1										; subtract #70 from a score
		BRzp LETTERD										; if zero or negative, the score was a D
		LD R5, FGRADE
		LD R7, SAVEREG7
		RET
...
LETTERA		LD R5, AGRADE								; Since R0 = #70, and A is ASCII #66, it is easier just to subtract 4 from 70 and assign that letter
		LD R7, SAVEREG7
		RET

LETTERB		LD R5, BGRADE								; Since R0 = #70, and B is ASCII #67, it is easier just to subtract 3 from 70 and assign that letter.
		LD R7, SAVEREG7
		RET

LETTERC		LD R5, CGRADE										; Since R0 = #70, and C is ASCII #68, it iss easier just to subtract 2 from 70 and assign that letter.
		LD R7, SAVEREG7
		RET

LETTERD		LD R5, DGRADE
		LD R7, SAVEREG7
		RET
