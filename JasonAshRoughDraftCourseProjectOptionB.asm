...
CHECKDONE
		LD R2, SCORESENTERED									; load the number of scores entered into R5 since FIRSTCHAR
		ADD R2, R2, #-5										; subtract 5 from R2
		BRzp NEXTFUNCT										; 5 test scores have been entered
...
MULTLOOP2	ADD R3, R3, #0										; add R3 to itself to check for multiplication done
		BRzp SAVE1STHUNS									; if the multiplication is done, branch to SAVE1STHUNS
		ADD R2, R2, R2										; add R2 to itself
		ADD R3, R3, #-1										; decrement the multiplication counter by one
...
SCOREDONE	
		AND R2, R2, #0										; clear R2
		LD  R3, CHAR1										; load the value from CHAR1 into R3
		...
		; ADD R2, R2, #0
		; add zero to R2 to set the status flags
		; BRn CANTBENEG
		; this shouldn't happen, but just in case the exam score is negative at this point
		...
		BRnzp CHECKDONE										; one score entered down, SCORESENTERED left to go!		
...
SCORESENTERED	.FILL		#0									; counter for number of test scores to enter
